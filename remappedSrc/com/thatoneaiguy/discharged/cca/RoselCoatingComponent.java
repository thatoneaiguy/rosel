...
import com.thatoneaiguy.discharged.init.DischargedDamageSources;
...
public class RoselCoatingComponent implements AutoSyncedComponent, CommonTickingComponent {
	...
	public int increment(int amount) {
		return level = Math.min(level + amount, 10);
	}

	public int decrement(int amount) {
		return level =  Math.max(level - amount, 0);
	}

	public int reset() {
		level = 0;
		Discharged.ROSEL_COATING_COMPONENT.sync(player);
		return level;
	}
	...
	public void tick() {
		...
		if ( ticks == delay ) {
			if ( level != 10 ) {
				delay = 20;
				ticks = 0;
				player.damage(DischargedDamageSources.CRYSTALISED, level * DischargedConfig.coating_multiplier);
				if ( player.isDead() ) {
					// Summon crystal entity
				}
			} else {
				player.damage(DischargedDamageSources.INSTAKILL, 10000);
				delay = 20;
				ticks = 0;
				level = 0;
			}
